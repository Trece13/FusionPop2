<%@ Page Title="" Language="C#" MasterPageFile="~/MDMasterPage.Master" AutoEventWireup="true" CodeBehind="ReprintDropPallets.aspx.cs" Inherits="whusap.WebPages.WorkOrders.ReprintDropPallets" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="Encabezado" runat="server">
    ...
    <style>
        ...
        #divPicketPending, #formPicking, #reasonLine, #MyEtiqueta, #lblReason, #ddReason, #bntChange,
...
<asp:Content ID="Content2" ContentPlaceHolderID="Contenido" runat="server">
                ...
                <form>
                    <div class="row">
                        <div class="col-12">
                            <div class="row">
                                <div class="col-6">
                                    <label for="ddPick">Picking</label>
                                </div>
                            </div>

                            <select id="ddPick" class="form-control">
                                <option value="0" selected>Select Picking</option>
                            </select>
                        </div>
                    </div>
                    <br />
                    <div class="row">
                        <div class="col-12">
                            <div class="row">
                                <div class="col-6">
                                    <label for="ddPaid">Pallet</label>
                                </div>
                            </div>

                            <select id="ddPaid" class="form-control">
                                <option value="0" selected>Select Pallet</option>
                            </select>
                        </div>
                    </div>
                    <br>
                    <div class="row">
                        <div class="col-6">
                            <button class="btn btn-primary col-12 btn-sm" id="btnReprintDrop" type="button">Reprint&nbsp;<span><i class="fas fa-circle-notch fa-spin" style="color: silver; display:none" id='dropLoader'></i></span></button>
    ...
    <script>
        ...
        var EventoAjax = function (Method, Data, MethodSuccess) {
            $.ajax({
                type: "POST",
                url: "ReprintDropPallets.aspx/" + Method.trim(),
                data: Data,
        ...
        var StartComponents = function () {
            ...
            var ddPick = document.getElementById("ddPick");
            var ddPaid = document.getElementById("ddPaid");
            ...
            var btnReprintDrop = document.getElementById("btnReprintDrop");
            ...
            btnReprintDrop.addEventListener("click", ReprintDrop);
            ddPick.addEventListener("change", GetPaids);
        ...
        var GetPicks = function () {
            ...
            EventoAjax("GetPicks", "{'consigment':'true'}", SucceessGetPicks);
        }
        ...
        var ReprintDrop = function () {
            skip = false;
            StartProcessing = false;
            //$('#StartLoader').show(100);
            EventoAjax("ReprintDrop", "{'PICK':'" + $("#ddPick option:selected").val() +"','PAID':'" + $("#ddPaid option:selected").val() + "','CWAR':'" + $("#ddPaid option:selected").attr('cwar') + "','MCNO':'" + $("#ddPaid option:selected").attr('mcno') + "','DATE':'" + $("#ddPaid option:selected").attr('date') + "'}", ReprintDropSuccess);
        }

        var ReprintDropSuccess = function (r) {
            var myObj = JSON.parse(r.d);
            if (myObj.PICK != ""){
                $("#Contenido_bcPick").attr("src", myObj.PICK);
                $("#lbMcno").html(myObj.MCNO)
                //JC 021021 Traer la cantidad del picking por pallet
                $("#lbQtyp").html(myObj.QTYT)
                $("#lbPaid").html(myObj.PAID)
        ...
        var GetPaids = function () {
            for (let i = ddPick.options.length; i > 0; i--) {
                ddPaid.remove(i);
            }
            EventoAjax("GetPaids", "{'PICK':'" + ddPick.value.trim() + "'}", GetPaidsSuccess);
        }

        var GetPaidsSuccess = function (r) {
